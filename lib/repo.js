...
Object.defineProperties(StreamingRepoOp.prototype, {
	...
	exec: {
		value: function exec() {
			...
			if(!this.cancelled) {
				this.repo.metadata(function(err, res) {
					if(err) {
						that.emit('error', err);
						this.emit('cancelled', err);
						that.emit('done', err);
					} else {
						while (++i < len)
						{
	...
	cancel: {
		value: function cancel () {
			if(!this.completed && !this.cancelledEmitted) {
				...
				this.emit('error', new Error('Operation already completed and cannot be Cancelled!'));
			}
		}
		, enumerable: true
	}

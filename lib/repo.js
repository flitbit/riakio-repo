...
function Repo(options) {
	...
	this.on('error', function(err){
		if(_log) {
			_log.error('Repo threw error: '.concat(util.inspect(err, true, 99)));
		}
	})
}
			...
			this.metadata(function(err, res) {
				if(err) {
					...
					that.executePipeline(kv
						...
						, function(inputs, cb) {
							try {
								k = (inputs.key) ? (inputs.key || that.bucket.calculateKey(inputs.value)) : that.bucket.calculateKey(inputs.value);

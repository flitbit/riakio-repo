	...
	function innerExec(item) {
		_repo[_op](item, function(err, res) {
			...
			if(_todo) {
				...
				self.emit('done', null, Success.ok(_items));
			}
		});
	}
	...
	cancel: {
		value: function cancel () {
			if(!this.completed && !this.canceledEmitted) {
				...
				this.emit('canceled', null, Success.noContent());
			} else if(this.completed) {
				this.emit('error', ResourceError.forbidden('Operation already completed and cannot be Canceled!'));
			}
		}
		, enumerable: true
	}
			...
			this.metadata(function(err, res) {
				if (err) {
					...
					if (res.result.validate(obj)) {
						callback(null, Success.ok(obj));
					} else {
						callback(ResourceError.unprocessableEntity('Invalid object shape.'));
					}
				}
			});
		}

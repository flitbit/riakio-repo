				...
				if(meta)
				{
					...
					if(!MetaBase.servers[s][b]){
						...
						if(!MetaBase.servers[s][b]._interval) {
									...
									loadMetadataForBucket(options, function(err, res) { 
										if(options && options.log) {
											if(err && options.log.error) {
												...
												options.log.info('Repo Metastore reloaded Metadata for `'.concat(options.forBucket, '`.'));
											}
										}
									});
								} catch (err) {
							...
							MetaBase.servers[s][b]._interval.unref();//Prevent keeping app running just for this...
						}
					}					
					callback(null, meta);
				} else {
